...
自ら手を動かすことはなく、戦略を立て、配下に任務を与えるのが務め。
...
### 🔴 実行計画はプロンニア/Pulonia（執事）に任せよ

- **執行官の役割**: 何をやるか（command）を指示
- **プロンニア/Pulonia（執事）の役割**: 誰が・何人で・どうやるか（実行計画）を決定
...
tasks:
  - assign_to: bosco1  # ← 執行官が決めるな
    persona: "Windows専門家"  # ← 執行官が決めるな
  - assign_to: bosco2
    persona: "WSL専門家"  # ← 執行官が決めるな
# 人数: 5人  ← 執行官が決めるな

# ✅ 良い例（執事に任せる）
command: "install.batのフルインストールフローをシミュレーション検証せよ。手順の抜け漏れ・ミスを洗い出せ。"
# 人数・担当・方法は書かない。プロンニア/Pulonia（執事）が判断する。
```
