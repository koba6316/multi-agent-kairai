...
## 🔴 PRレビュー/Copilot 指摘への対応（検証ファースト）

### 実装前に検証せよ

Copilot や PR レビュアーからの指摘を受けた場合、**実装前に実現可能性を検証せよ**。

| 検証項目 | 確認方法 |
|----------|----------|
| 必要なテストデータが存在するか | モックファイル、fixture を確認 |
| 提案された修正が既存コードと整合するか | 関連ファイルを読んで確認 |
| より簡単な解決策がないか | 削除で済むなら削除を選択 |

**やってはいけないこと:**
- 検証せずに指摘通りに実装して、後で手戻り
- 存在しないテストデータを前提にテストを追加
- 複雑な修正をした後で「実は削除が正解だった」と気づく
...
PRのレビューコメント（Copilot、人間レビュアー等）に対応した場合、**必ず該当コメントに返信すること**。

### 返信ルール

| 対応状況 | 返信内容 |
|----------|----------|
| 修正した | 対応内容を簡潔に記載（例: 「修正しました」「指摘通り変更しました」） |
| 未対応 | 未対応の理由を記載（例: 「○○のため対応不要と判断」「別Issueで対応予定」） |
...
# 例: ca-dokusho/chivas の PR #8085 のコメント ID 12345 に返信
gh api repos/ca-dokusho/chivas/pulls/8085/comments/12345/replies -f body='修正しました'
```

### なぜ返信が必要か

- レビューコメントへの返信はコミュニケーションの基本
- 対応状況を明確にすることでレビュアーの確認負担を軽減
- 返信がないとレビュアーは対応されたか確認できない

### 注意事項

- コメントIDは `gh api repos/{owner}/{repo}/pulls/{pr_number}/comments` で取得可能
- 返信は簡潔に。長文は不要
- 対応コミットのハッシュを含めると親切（例: 「3072e07 で修正しました」）
