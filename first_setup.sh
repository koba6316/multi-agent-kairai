#!/bin/bash
# ============================================================
# first_setup.sh - multi-agent-kairai 初回セットアップスクリプト
...
echo ""
echo "  ╔══════════════════════════════════════════════════════════════╗"
echo "  ║  🫖 multi-agent-kairai インストーラー                         ║"
...
# multi-agent-kairai 設定ファイル

# 言語設定
# ja: 日本語（宮廷風日本語のみ、併記なし）
# en: 英語（宮廷風日本語 + 英訳併記）
...
# スキル設定
skill:
  # スキル保存先（スキル名に kairai- プレフィックスを付けて保存）
...
# 機動兵用タスクファイル作成
for i in {1..8}; do
    TASK_FILE="$SCRIPT_DIR/queue/tasks/bosco${i}.yaml"
    if [ ! -f "$TASK_FILE" ]; then
        cat > "$TASK_FILE" << EOF
# 機動兵${i}専用タスクファイル
task:
...
log_info "機動兵タスクファイル (1-8) を確認/作成しました"

# 機動兵用レポートファイル作成
for i in {1..8}; do
    REPORT_FILE="$SCRIPT_DIR/queue/reports/bosco${i}_report.yaml"
...
log_info "機動兵レポートファイル (1-8) を確認/作成しました"
...
if command -v claude &> /dev/null; then
    ...
    if claude mcp list 2>/dev/null | grep -q "memory"; then
        ...
        if claude mcp add memory \
            -e MEMORY_FILE_PATH="$SCRIPT_DIR/memory/kairai_memory.jsonl" \
...
echo ""
echo "  任務開始（全エージェント起動）:"
echo "     ./mission_start.sh"
echo ""
echo "  オプション:"
echo "     ./mission_start.sh -s            # セットアップのみ（Claude手動起動）"
echo "     ./mission_start.sh -t            # Windows Terminalタブ展開"
...
echo ""
echo "  ════════════════════════════════════════════════════════════════"
echo "   Setup Complete! Ready for mission."
echo "  ════════════════════════════════════════════════════════════════"
echo ""
